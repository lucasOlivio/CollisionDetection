...
VAOManager::VAOManager(iShaderMediator* pShaderMediator)
{
...
bool VAOManager::LoadModelIntoVAO(std::string& fileName,
...

{
    ...
    GLint vpos_location = this->m_pShaderMediator->GetAL(shaderProgram, "vPos");
    GLint vcol_location = this->m_pShaderMediator->GetAL(shaderProgram, "vCol");
    GLint vNormal_location = this->m_pShaderMediator->GetAL(shaderProgram, "vNormal");
...
engine::sMesh* VAOManager::FindMeshByModelName(std::string& filename)
{
    using namespace engine;
...
bool VAOManager::m_LoadTheFile_Ply_XYZ_N_RGBA(std::string theFileName, engine::sMesh* pMesh)
{
    using namespace engine;
